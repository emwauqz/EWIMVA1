...
export default function EwimvaSuit(): JSX.Element {
...
const fetchProducts = async () => {
    try {
    const response = await fetch('http://localhost:3001/products?category=Костюмы&gender=male');
    if (!response.ok) {
        throw new Error('Ошибка загрузки костюмов');
    }
    ...
    setError('Не удалось загрузить костюмы. Попробуйте позже.');
    } finally {
    setLoading(false);
    }
};

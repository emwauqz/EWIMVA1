...
export default function EwimvaJacketsOuterwear(): JSX.Element {
...
const fetchProducts = async () => {
    try {
    const response = await fetch('http://localhost:3001/products?category=Куртки&gender=male');
    if (!response.ok) {
        throw new Error('Ошибка загрузки курток');
    }
    const data = await response.json();
    // Преобразуем данные для соответствия интерфейсу Product
    const formattedData: Product[] = data.map((item: any) => ({
        ...
        price: item.price,
        image: item.image,
        colorVariants: item.colorVariants || [], // Убедимся, что colorVariants - массив
        gender: item.gender,
    }));
    setProducts(formattedData);
    } catch (err) {
    setError('Не удалось загрузить куртки. Попробуйте позже.');
    } finally {
    setLoading(false);
    }
};

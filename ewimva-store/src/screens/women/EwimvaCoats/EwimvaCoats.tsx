...
const fetchProducts = async () => {
    try {
    const response = await fetch('http://localhost:3001/products?category=Пальто&gender=female');
    if (!response.ok) {
        throw new Error('Ошибка загрузки пальто');
    }
    ...
    setError('Не удалось загрузить пальто. Попробуйте позже.');
    } finally {
    setLoading(false);
    }
};

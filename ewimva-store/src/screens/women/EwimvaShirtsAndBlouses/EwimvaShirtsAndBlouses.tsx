...
const fetchProducts = async () => {
    try {
    const response = await fetch('http://localhost:3001/products?category=Рубашки и футболки&gender=female');
    if (!response.ok) {
        throw new Error('Ошибка загрузки рубашек и футболок');
    }
    ...
    setError('Не удалось загрузить рубашки и футболки. Попробуйте позже.');
    } finally {
    setLoading(false);
    }
};

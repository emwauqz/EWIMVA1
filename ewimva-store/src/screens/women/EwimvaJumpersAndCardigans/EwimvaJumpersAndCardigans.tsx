...
const fetchProducts = async () => {
    try {
    const response = await fetch('http://localhost:3001/products?category=Джемперы и кардиганы&gender=female');
    if (!response.ok) {
        throw new Error('Ошибка загрузки джемперов и кардиганов');
    }
    ...
    setError('Не удалось загрузить джемперы и кардиганы. Попробуйте позже.');
    } finally {
    setLoading(false);
    }
};
...
<>
    ...
    <main className="flex-1">
        ...
        <div className="p-8 text-center font-['Montserrat'] text-[24px] text-[#131313]">
            Джемперы и кардиганы не найдены
        </div>
        )}
    </main>
    </div>
